...
import { createMyDoctorScheduleAction } from "@/app/(dashboardLayout)/doctor/dashboard/schedules/_action";
...
const BookScheduleModal = () => {
  ...
  const availableSchedules = useMemo<TSchedule[]>(() => {
    const now = new Date();
    ...
    const existingScheduleIds = new Set(
      (mySchedulesResponse?.data ?? []).map(
        (item: TDoctorSchedule) => item.scheduleId,
      ),
    );
