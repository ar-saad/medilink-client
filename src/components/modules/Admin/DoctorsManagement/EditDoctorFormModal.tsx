...
import { updateDoctorAction } from "@/app/(dashboardLayout)/admin/dashboard/doctors/_action";
...
import { Gender, TDoctor, TUpdateDoctorPayload } from "@/types/doctor.types";
import { TSpecialty } from "@/types/specialty.types";
import {
  editDoctorFormZodSchema,
  TEditDoctorFormValues,
...
const getInitialValues = (doctor: TDoctor | null): TEditDoctorFormValues => ({
  name: doctor?.name ?? "",
...
const EditDoctorFormModal = ({
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const specialtyChanges: TUpdateDoctorPayload["specialties"] = [];
      ...
      const payload: TUpdateDoctorPayload = {
        doctor: {
          name: value.name,
                    ...
                    return (
                      <div className="space-y-1.5">
                        ...
                        <Select
                          ...
                          onValueChange={(value) => {
                            field.handleChange(
                              value as TEditDoctorFormValues["gender"],
                            );
                            field.handleBlur();
                          }}
                        >
