...
import { updateSpecialty } from "@/services/specialty.services";
...
interface UpdateSpecialtyFormModalProps {
  specialty: TSpecialty | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

const UpdateSpecialtyFormModal = ({
  specialty,
  open,
  onOpenChange,
}: UpdateSpecialtyFormModalProps) => {
  const queryClient = useQueryClient();
  const [preview, setPreview] = useState<string | null>(null);

  useEffect(() => {
    if (specialty?.icon) {
      setPreview(specialty.icon);
    } else {
      setPreview(null);
    }
  }, [specialty]);
  ...
  const updateMutation = useMutation({
    mutationFn: (formData: FormData) => updateSpecialty(specialty!.id, formData),
    onSuccess: () => {
  ...
  // Sync form values when specialty changes
  useEffect(() => {
    if (specialty) {
      form.reset({
        title: specialty.title,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Update Specialty</DialogTitle>
        </DialogHeader>
        <form
          onSubmit={(e) => {
            e.preventDefault();
...
export default UpdateSpecialtyFormModal;
